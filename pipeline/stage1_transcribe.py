...
def _apply_vad(audio: np.ndarray, sr: int, threshold: float = 0.3) -> np.ndarray:
    """
    Strip leading/trailing silence using silero-VAD.
    Merges all detected speech segments so mid-clip pauses are kept.
    Falls back to raw audio if silero-VAD is unavailable.

    IMPORTANT: silero-VAD always runs at 16 kHz internally. The returned
    sample indices are in 16 kHz space. We convert them back to seconds,
    then to the original sample rate before slicing.
    """
    VAD_SR = 16000
    original_sr = sr
    ...
    try:
        ...
        if original_sr != VAD_SR:
            import torchaudio
            audio_16k = torchaudio.functional.resample(audio_tensor, original_sr, VAD_SR)
        else:
            audio_16k = audio_tensor

        timestamps = get_speech_timestamps(
            audio_16k, vad_model, threshold=threshold, sampling_rate=VAD_SR
        )
        ...
        # Convert 16 kHz sample indices → seconds → original SR sample indices
        start_sec = timestamps[0]["start"] / VAD_SR
        end_sec   = timestamps[-1]["end"]   / VAD_SR

        # Add 100ms padding so we don't clip words at edges
        start_sec = max(0.0, start_sec - 0.1)
        end_sec   = min(len(audio) / original_sr, end_sec + 0.1)

        start_sample = int(start_sec * original_sr)
        end_sample   = int(end_sec   * original_sr)

        clipped = audio[start_sample:end_sample]
        logger.info(
            f"VAD: kept {end_sec - start_sec:.2f}s of "
            f"{len(audio)/original_sr:.2f}s total "
            f"({len(timestamps)} speech segment(s))"
        )
        return clipped
