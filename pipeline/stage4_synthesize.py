...
# ---------------------------------------------------------------------------
# Ensure ffmpeg is on PATH (winget installs to a non-default location)
# ---------------------------------------------------------------------------
def _ensure_ffmpeg_path() -> None:
    """Add common ffmpeg install directories to PATH if not already present."""
    ...
    # Also search AppData\Local\Microsoft\WinGet\Packages
    winget_base = Path.home() / "AppData" / "Local" / "Microsoft" / "WinGet" / "Packages"
    if winget_base.exists():
        for pkg in winget_base.glob("Gyan.FFmpeg*"):
            for ffmpeg_bin in pkg.glob("**/bin"):
                if (ffmpeg_bin / "ffmpeg.exe").exists():
                    candidates.insert(0, str(ffmpeg_bin))

    for path in candidates:
        if Path(path).exists() and path not in os.environ.get("PATH", ""):
            os.environ["PATH"] = path + os.pathsep + os.environ.get("PATH", "")
            if shutil.which("ffmpeg"):
                logging.getLogger(__name__).info(f"ffmpeg found via PATH injection: {path}")
                return
